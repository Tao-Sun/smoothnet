...
def get_segnet(images, images_shape, labels):
    num_filter = 64
    ...
    _, out_shapes, _ = conv_decode1.infer_shape(data=images_shape)
    print("Layer conv_decode1 output shape: " + str(out_shapes))

    conv_classifier = mx.sym.Convolution(data=conv_decode1, num_filter=11, kernel=(1, 1), pad=(0, 0),
                                         stride=(1, 1), workspace=2048)
    _, out_shapes, _ = conv_classifier.infer_shape(data=images_shape)
    print("Layer conv_classifier output shape: " + str(out_shapes))

    segnet = mx.sym.SoftmaxOutput(data=conv_classifier, label=labels, ignore_label=11, multi_output=True, use_ignore=True)
    _, out_shapes, _ = segnet.infer_shape(data=images_shape)
